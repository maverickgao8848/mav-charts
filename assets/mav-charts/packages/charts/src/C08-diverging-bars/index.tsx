...
export type DivergingBarChartProps = { data?: readonly DivergingBarDatum[]; visualSystem?: VisualSystemId; animate?: boolean; durationMs?: number; progress?: number; title?: string; subtitle?: string; seriesName?: string; unit?: string };
...
export function DivergingBarChart({ durationMs, progress,  data = divergingBarExample, visualSystem = "signal", animate, title = "Growth and decline split the map", subtitle = "SIGNED CHANGE · SHARED ZERO BASELINE", seriesName = "Change", unit = "" }: DivergingBarChartProps) { animate = progress === undefined ? animate : false; const theme = getVisualSystem(visualSystem); const shouldAnimate = resolveDivergingBarAnimation(animate, usePrefersReducedMotion()); const validation = validateDivergingBarData(data); const state = data.length === 0 ? "empty" : validation.valid ? "ready" : "invalid"; return <ChartShell durationMs={durationMs} progress={progress} code="C08" title={title} subtitle={subtitle} source={`${theme.name.toUpperCase()} · DIVERGING BARS`} theme={theme} state={state} description="Signed values extend from one shared zero baseline while missing values remain gaps."><DivergingBarGeometry data={validation.valid ? data : []} theme={theme} animate={shouldAnimate} seriesName={seriesName} unit={unit} /></ChartShell>; }
